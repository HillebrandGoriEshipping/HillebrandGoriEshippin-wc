...
curl_setopt_array($curl, array(
  CURLOPT_URL => "https://test.eshipping.hillebrandgori.app/api/shipment/create",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
...
function sendConfirmationMail($order_id, $to, $name, $type_liv)
{
  ...
  $subject = '[' . get_option('woocommerce_email_from_name') . '] ' . __('Your order is ready for shipment.', 'HillebrandGoriEshipping');
  $body = '<h5>' . __('Hello, ', 'HillebrandGoriEshipping') . $name . '</h5>';
  $body .= '<p>' . __('Thanks for your order.', 'HillebrandGoriEshipping') . '</p>';
  $body .= '<p>' . __('Your order is now awaiting for shipment :', 'HillebrandGoriEshipping') . '</p>';
  $body .= '<p>' . __('Chosen carrier :', 'HillebrandGoriEshipping') . ' ' . strtoupper($offer_name) . '</p>';
  $body .= '<p>' . __('Order tracking number(s) :', 'HillebrandGoriEshipping') . '<ul><li>' . str_replace(';', '</li><li>', $result[0]['tracking_code']) . '</li></ul></p>';
  if ($result[0]['tracking_link']) {
    $body .= '<p>' . __('You can follow your order by clicking on ', 'HillebrandGoriEshipping') . ' <a href="' . $result[0]['tracking_link'] . '">' . __('this link', 'HillebrandGoriEshipping') . '</a></p>';
  }
  $body .= '<table><th>' . __('Billing address', 'HillebrandGoriEshipping') . '</th>';
  $body .= '<th>' . __('Shipping address', 'HillebrandGoriEshipping') . '</th>';
  ...
  $body .= '<p>' . __('Regards,', 'HillebrandGoriEshipping') . '<br>' . get_option('woocommerce_email_from_name') . '.</p>';
