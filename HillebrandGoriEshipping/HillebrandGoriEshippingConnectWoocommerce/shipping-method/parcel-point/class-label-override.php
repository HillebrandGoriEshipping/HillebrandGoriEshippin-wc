...
use HillebrandGoriEshipping\HillebrandGoriEshippingConnectWoocommerce\Util\Shipping_Rate_Util;
use HillebrandGoriEshipping\HillebrandGoriEshippingConnectWoocommerce\Util\Product_Util;
...
class Label_Override
{
	...
	function rename_shipping_order_item_totals($order, $order_glob)
	{
		...
		if (preg_match("/HillebrandGoriEshipping/i", $shipping)) {
			global $wpdb;
			...
			$bodytag = str_replace("HillebrandGoriEshipping", $offres["name"] ?? '', $shipping);
	...
	function wf_hide_shipping_method_based_on_shipping_class($available_shipping_methods, $package)
	{
		...
		foreach ($available_shipping_methods as $key => $value) {
			if ($value->get_method_id() !== null) {
				if (($value->get_method_id() == 'HillebrandGoriEshipping_connect' && WC()->cart->cart_contents_count < (int)get_option('VINW_NBR_MIN')) || ($value->get_method_id() == 'HillebrandGoriEshipping_connect' && $nbr_Bottles_magnums == false)) {
	...
	public function action_woocommerce_before_checkout_form($wccm_autocreate_account)
	{
		...
		<script src="' . get_option('siteurl') . '/wp-content/plugins/HillebrandGoriEshipping/HillebrandGoriEshipping/HillebrandGoriEshippingConnectWoocommerce/assets/js/expedition-picker.js"></script>';
	}
	...
	private function get_parcelpoint_address($parcelpoint)
	{
		...
		if (null !== $parcelpoint->distance) {
			...
			$result  .= ' (' . sprintf(__('%skm away', 'HillebrandGoriEshipping'), $distance) . ')';
		}
	...
	{
		$url = get_option('siteurl') . '/wp-content/plugins/HillebrandGoriEshipping/HillebrandGoriEshipping/HillebrandGoriEshippingConnectWoocommerce/assets/img/';
		switch ($offer) {
			case 'ups':
				$url .= "ups.png";
				break;
			...
			if (isset($chaine)) {
				...
				if ($tab[4] == "pointRelais" && (get_option("VINW_PREF_STAT") == "pointRelais" || get_option("VINW_PREF_STAT") == "les deux") && in_array('ups', get_option('VINW_PREF_TRANSP'))) {
					$fields['billing']['point_relais'] = array(
						'type'      => 'hidden',
						'label'     => __('Pickup point:', 'HillebrandGoriEshipping'),
						'required'  => true,
						'class'     => array(''),
						'clear'     => true
					);
	...
	{
		$chosen_parcel_point = Controller::get_chosen_point('HillebrandGoriEshipping_connect');
		$name = '';
		if ($chosen_parcel_point !== null) {
			$name = $chosen_parcel_point->name;
		}
		...
		$full_label .= '<br><br><span class="VINW-select-parcel active-relay" id="relayaction">' . __('Choose pickup point', 'HillebrandGoriEshipping') . '</span>';
		$full_label .= '<br><br><span class="VINW-parcel-client">' . __('Your pickup point:', 'HillebrandGoriEshipping') . ' <span class="VINW-parcel-name">' . $name . '</span></span>';
		$full_label .= '</div>';
...
<?php }
	...
	public function change_shipping_label($full_label, $method)
	{
		...
		if ($method->method_id  == 'HillebrandGoriEshipping_connect' && $nbr_Bottles_magnums != false && is_checkout()) {
			...
			if (WC()->session->customer['country'] && (WC()->session->customer['shipping_postcode'] || WC()->session->customer['country'] == 'AE') && WC()->session->customer['shipping_city']) {
				if (WC()->session->get('VINW_CONF_Colis') !== null) {
					...
					if (!$isFeasable) {
						$full_label .= '<br><span class="notif">' . __("Max number of bottles exceeded to deliver to a private individual in this destination country.", "HillebrandGoriEshipping") . ' </span>';
						return $full_label;
					}
				...
				if (WC()->session->get('VINW_CONF_Colis') !== null) {
					if (is_countable($filtered_responses) && count($filtered_responses) > 0) {
						...
						$offre3 .= '<p>' . __("No offers exist", "HillebrandGoriEshipping") . '</p> ';
						$offre3 .= '<div class="offers" style="display:none;" id="offrenotexiste"> ';
					}
				}
				...
				if (WC()->session->get('VINW_CONF_Colis') !== null) {
					...
					$offre1 .= '<span style="color: black;font-weight: 600;margin-left: 10px;">' . __('Available offers', 'HillebrandGoriEshipping') . ':</span>';
					...
					foreach ($reponse as $key => $offer) {
						...
						if ($destiType == "company" && ($expedition_type == "export" || $expedition_type == "fiscal_rep")) {
							$full_label = '<br><span class="notif">' . __("If you are a company, please contact the sender.", "HillebrandGoriEshipping") . ' </span>';
							break;
						}
						...
						if (preg_match('/UPS Access Point Economy/', $offer['service']) == 0 && preg_match('/Chrono Relais 13H/', $offer['service']) == 0) {
							...
							$offre1 .= '<p  id="offer-cont_' . $key . '" for="shipping_method_offer_' . $key . '"><strong>' . $offer['service'] . '</strong><br>  ' . __('Price: ', 'HillebrandGoriEshipping') . ' <strong>' . sprintf("%01.2f", $finalPrice) . '€</strong> <br> | ' . __('Estimated delivery', 'HillebrandGoriEshipping') . ' ' . $date1 . '<br> </p></div>';
					...
					foreach ($reponse as $key => $offer) {
						...
						if (preg_match('/UPS Access Point Economy/', $offer['service']) == 1 || preg_match('/Chrono Relais 13H/', $offer['service']) == 1) {
							...
							$offre2 .= '<p  id="offer-cont_' . $key . '" for="shipping_method_offer_' . $key . '"><strong>' . $offer['service'] . '</strong><br>  ' . __('Price: ', 'HillebrandGoriEshipping') . '  <strong>' . sprintf("%01.2f", $finalPrice) . '€</strong> <br>| ' . __('Estimated delivery', 'HillebrandGoriEshipping') . ' ' . $date2 . '<br> </p></div>';
						}
						$i++;
					}
				...
				$full_label .= '<div class="appointment-link" style="margin-left: 10px;" id="container-method-HillebrandGoriEshipping-link">';
				...
				if (WC()->session->get('VINW_CONF_Colis') !== null) {
					...
					if (get_option("VINW_PREF_STAT") == "domicile") {
						if (!empty($reponse)) {
							$full_label .= '<input type="radio" name="shipping_mettype[]" id="shipping_method_0_HillebrandGoriEshipping_connect_dom" value="domicile" class="shipping_method">';
							$full_label .= '<label for="shipping_method_0_HillebrandGoriEshipping_connect_dom">' . __('Domestic', 'HillebrandGoriEshipping') . '</label>';
					...
					if (get_option("VINW_PREF_STAT") == "les deux") {
						if (!empty($reponse)) {
							if (($compOffreUps > 0 || $compOffreChrono > 0) && get_option("VINW_MAPBOX_ACCESS_KEY") && get_option("mapbox-api-key-validate") == "1") {
								$full_label .= '<input type="radio" name="shipping_mettype[]"  id="shipping_method_0_HillebrandGoriEshipping_connect_pr" value="pointRelais" class="shipping_method">';
								$full_label .= '<label for="shipping_method_0_HillebrandGoriEshipping_connect_pr">' . __('Pickup point', 'HillebrandGoriEshipping') . '</label>';
							...
							$full_label .= '<br><input type="radio" name="shipping_mettype[]"  id="shipping_method_0_HillebrandGoriEshipping_connect_dom" value="domicile"   class="shipping_method" >';
							$full_label .= '<label for="shipping_method_0_HillebrandGoriEshipping_connect_dom">' . __('Domestic', 'HillebrandGoriEshipping') . '</label>';
					...
					if (get_option("VINW_PREF_STAT") == "pointRelais" && (in_array('ups', get_option('VINW_PREF_TRANSP')) || in_array('chronopost', get_option('VINW_PREF_TRANSP'))) && get_option("VINW_MAPBOX_ACCESS_KEY") && get_option("mapbox-api-key-validate") == "1") {
						if ($compOffreUps > 0 || $compOffreChrono > 0) {
							$full_label .= '<br><input type="radio" name="shipping_mettype[]"  id="shipping_method_0_HillebrandGoriEshipping_connect_pr" value="pointRelais" class="shipping_method">';
							$full_label .= '<label for="shipping_method_0_HillebrandGoriEshipping_connect_pr">' . __('Pickup point', 'HillebrandGoriEshipping') . '</label>';
				...
				if (WC()->session->get('VINW_CONF_Colis')) {
					if (!empty($reponse)) {
						...
						if (get_option("VINW_PREF_STAT") == "pointRelais" && (in_array('ups', get_option('VINW_PREF_TRANSP')) || in_array('chronopost', get_option('VINW_PREF_TRANSP'))) && ($compOffreUps > 0 || $compOffreChrono > 0) && get_option("VINW_MAPBOX_ACCESS_KEY") && get_option("mapbox-api-key-validate") == "1") {
							// $full_label .= '<br><input type="button" value="' . __('Confirm my shipping choice', 'HillebrandGoriEshipping') . '"  id="validate">';
							if (is_checkout() && isset($chaine)) {
								$tab = explode(";", $chaine);

								if ($tab[4] == "pointRelais") {
									$full_label .= '<div class="relay"><br><br><span class="VINW-select-parcel active-relay" id="relayaction">' . __('Choose pickup point', 'HillebrandGoriEshipping') . '</span>';
									$full_label .= '<br><br><span class="VINW-parcel-client" ' . $style . '>' . __('Your pickup point:', 'HillebrandGoriEshipping') . ' <span class="VINW-parcel-name">' . $name . '</span></span></div>';
								}
							}
						} elseif (get_option("VINW_PREF_STAT") == "domicile" || get_option("VINW_PREF_STAT") == "les deux") {
							// $full_label .= '<br><br><input type="button" value="' . __('Confirm my shipping choice', 'HillebrandGoriEshipping') . '" id="validate">';
							if (is_checkout() && isset($chaine)) {
								$tab = explode(";", $chaine);
							...
							$full_label .= '<br><span class="notif">' . __("No shipping offer found for this address, please check your shipping informations or choose another shipping method.", "HillebrandGoriEshipping") . ' </span>';
						}
					} else {
						WC()->session->set('VINW_CONF_EXP', null);
						WC()->cart->calculate_totals();
				...
				$full_label .= '<br><span class="notif"> ' . __('Please enter your adress', 'HillebrandGoriEshipping') . ' </span>';
			}
		}

		//Checks if the $method object's method_id is equal to 'HillebrandGoriEshipping_connect', $nbr_Bottles_magnums is not false, and is_checkout() is false. If all conditions are met, it appends a notification message to the $full_label variable.
		if ($method->method_id  == 'HillebrandGoriEshipping_connect' && $nbr_Bottles_magnums != false && !is_checkout()) {
			$full_label .= '<br><br><span class="notif"> ' . __('You can choose your shipment on the next page', 'HillebrandGoriEshipping') . ' </span>';
		}
		return $full_label;
	}
}
