...
use HillebrandGoriEshipping\HillebrandGoriEshippingConnectWoocommerce\Util\Customer_Util;
...
use HillebrandGoriEshipping\HillebrandGoriEshippingConnectWoocommerce\Util\Parcelpoint_Util;
...
class Controller
{
	...
	// 	wp_enqueue_script( 'VINW_shipping_EXP', $this->plugin_url . 'HillebrandGoriEshipping/HillebrandGoriEshippingConnectWoocommerce/assets/js/expedition-picker.js', array(), $this->plugin_version, true );
	// }
	public function parcel_point_scripts()
	{
		...
		if ($this->is_checkout_or_cart()) {
			$translations = array(
				'error' => array(
					'carrierNotFound' => __('Unable to find carrier', 'HillebrandGoriEshipping'),
					'addressNotFound' => __('Could not find address', 'HillebrandGoriEshipping'),
					'mapServerError'  => __('Could not connect to map server', 'HillebrandGoriEshipping'),
				),
				'text'  => array(
					'openingHours'        => __('Opening hours', 'HillebrandGoriEshipping'),
					'chooseParcelPoint'   => __('Choose pickup point', 'HillebrandGoriEshipping'),
					'yourAddress'         => __('', 'HillebrandGoriEshipping'),
					'closeMap'            => __('Close map', 'HillebrandGoriEshipping'),
					'selectedParcelPoint' => __('Your pickup point:', 'HillebrandGoriEshipping'),
					/* translators: %s: distance in km */
					'kmaway'              => __('%skm away', 'HillebrandGoriEshipping')
				),
				'day'   => array(
					'MONDAY'    => __('monday', 'HillebrandGoriEshipping'),
					'TUESDAY'   => __('tuesday', 'HillebrandGoriEshipping'),
					'WEDNESDAY' => __('wednesday', 'HillebrandGoriEshipping'),
					'THURSDAY'  => __('thursday', 'HillebrandGoriEshipping'),
					'FRIDAY'    => __('friday', 'HillebrandGoriEshipping'),
					'SATURDAY'  => __('saturday', 'HillebrandGoriEshipping'),
					'SUNDAY'    => __('sunday', 'HillebrandGoriEshipping'),
				),
			);

			wp_enqueue_script('VINW_promise_polyfill', $this->plugin_url . 'HillebrandGoriEshipping/HillebrandGoriEshippingConnectWoocommerce/assets/js/promise-polyfill.min.js', array(), $this->plugin_version);
			wp_enqueue_script('VINW_polyfills', $this->plugin_url . 'HillebrandGoriEshipping/HillebrandGoriEshippingConnectWoocommerce/assets/js/polyfills.min.js', array(), $this->plugin_version);
			...
			wp_enqueue_script('VINW_shipping', $this->plugin_url . 'HillebrandGoriEshipping/HillebrandGoriEshippingConnectWoocommerce/assets/js/parcel-point.min.js', array('VINW_mapbox_gl', 'VINW_polyfills', 'VINW_promise_polyfill'), $this->plugin_version);
	...
	public function parcel_point_styles()
	{
		if ($this->is_checkout_or_cart()) {
			...
			wp_enqueue_style('VINW_parcel_point', $this->plugin_url . 'HillebrandGoriEshipping/HillebrandGoriEshippingConnectWoocommerce/assets/css/parcel-point.css', array(), $this->plugin_version);
		}
	}
	...
	public static function get_network_options()
	{
		...
		foreach ($networks as $network => $carrier_array) {
			...
			$options[$network] = sprintf(__('Pickup point map including %s', 'HillebrandGoriEshipping'), implode(', ', $carrier_array));
		}
	...
	public function get_points_callback()
	{
		...
		if (!isset($_REQUEST['carrier'])) {
			wp_send_json_error(array('message' => __('Unable to find carrier', 'HillebrandGoriEshipping')));
		}
		// phpcs:ignore
	...
	public static function init_points($address, $method)
	{
		...
		if ('HillebrandGoriEshipping_connect' !== Shipping_Rate_Util::get_method_id($method)) {
