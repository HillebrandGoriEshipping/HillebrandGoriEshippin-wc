...
use HillebrandGoriEshipping\HillebrandGoriEshippingConnectWoocommerce\Util;
...
class Admin_Order_Page
{
	...
	function action_before_save_order_item_boutton($order_id)
	{
		...
		foreach ($order->get_items('shipping') as $item_id => $shipping_item_obj) {
			...
			if ($shipping_method_id == 'HillebrandGoriEshipping_connect') {
				$query = "SELECT * FROM {$wpdb->prefix}VINW_order_expidition WHERE order_id = '" . $order_id . "'";
				//phpcs:ignore
				$result = $wpdb->get_results($query, ARRAY_A);
				...
				if ($type_package == "colis") {
					if (is_countable($get_rates) && count($get_rates) > 0) {
						...
						$offre3 .= '<p>' . __("No offer available", "HillebrandGoriEshipping") . '</p> ';
						$offre3 .= '</td></tr> ';
					}
				}
				...
				<tr>
					<th>' . __('Nb.Package', 'HillebrandGoriEshipping') . '</th>
					<th>' . __('Nb.Bottles', 'HillebrandGoriEshipping') . '</th>
					<th>' . __('Width', 'HillebrandGoriEshipping') . '</th>
					<th>' . __('Height', 'HillebrandGoriEshipping') . '</th>
					<th>' . __('length', 'HillebrandGoriEshipping') . '</th>
					<th>' . __('weight', 'HillebrandGoriEshipping') . '</th>
				</tr>';
					...
					for ($l = 0; $l < count($package); $l++) {
						...
						if (isset($package[$l]['nbBottles'])) {
							$fields1 .= "<td>" . $package[$l]['nbBottles'] . ' ' . __('Bottles', 'HillebrandGoriEshipping') . '</td>';
						} else {
...
?>
				<tbody id="offre">
					...
					<tr class="shipping" data-order_item_id="19">
						<td colspan="3">
							<strong><?php esc_html_e('Price difference:', 'HillebrandGoriEshipping') ?></strong>
							<?php
							if ($diffrence > 0) { ?>
					...
					<?php if ($offres != null) { ?>
						<tr class="shipping" data-order_item_id="19">
							<td colspan="3">
								<strong><?php esc_html_e('Your expedition method:', 'HillebrandGoriEshipping') ?> </strong>
								...
								if ($type_livraison == "domicile") {
									esc_html_e('Domestic', 'HillebrandGoriEshipping');
								} else {
									esc_html_e('Relay point', 'HillebrandGoriEshipping');
								}	 ?>
								<br /><br />
								<strong> <?php esc_html_e('Chosen offer:', 'HillebrandGoriEshipping') ?> </strong><?php echo $offres['service'] ?>
								<br /><br />
								<strong><?php esc_html_e('Delivery date:', 'HillebrandGoriEshipping') ?> </strong><?php echo $offres['deliveryDate'] ?> at <?php echo $offres['pickupTime'] ?>
								<?php if (get_option('VINW_TAX_RIGHTS') == "dest"  && $expedition_type == "export") { ?>
									<br /><br />
									<strong><?php esc_html_e('Estimated tax and duties:', 'HillebrandGoriEshipping'); ?></strong>
								<?php echo $tax_amount . " " . $currency;
									esc_html_e(' at the recipient expense', 'HillebrandGoriEshipping');
								}
								?>
							</td>
							<td colspan="3">
								<table id="shipping-details-table" style="width: 100%; border: none;">
									...
									<thead>
										<tr>
											<td style="border-top: none;"><strong><?php esc_html_e('Shipping costs details:', 'HillebrandGoriEshipping') ?></strong></td>
										</tr>
									</thead>
									<tbody>
										<!-- EXPORT -->
										<?php if ($expedition_type == "export") { ?>
											<tr>
												<th><strong> <?php esc_html_e('Transport:', 'HillebrandGoriEshipping') ?> </strong></th>
												<td style="border: none;"><?php echo number_format($offres['price'], 2, ',', ' ') . " €"; ?></td>
											</tr>
											<?php if (get_option('VINW_TAX_RIGHTS') == "exp") { ?>
												<tr>
													<td><strong> <?php esc_html_e('Estimated tax and duties:', 'HillebrandGoriEshipping') ?> </strong></td>
													<td style="border: none;"><?php echo $tax_amount . " " . $currency; ?>
														<?php
														if (get_option('VINW_TAX_RIGHTS') == "exp") {
															esc_html_e(' at your expense (included in shipping price)', 'HillebrandGoriEshipping');
														} else {
															esc_html_e(' at the recipient expense', 'HillebrandGoriEshipping');
														}
														?>
										...
										<?php } elseif ($expedition_type == "fiscal_rep") { ?>
											...
											<tr>
												<th><strong> <?php esc_html_e('VAT on transport:', 'HillebrandGoriEshipping'); ?> </strong></th>
												<td style="border: none;"><?php echo number_format($vat_transport, 2, ',', ' ') . " €"; ?></td>
											</tr>
											<tr>
												<?php $accises_excl_tax = $tax_amount - $vat_transport - $vat_accises; ?>
												<th><strong> <?php esc_html_e('Excise duty:', 'HillebrandGoriEshipping'); ?> </strong></th>
												<td style="border: none;"><?php echo number_format($accises_excl_tax, 2, ',', ' ') . " €"; ?></td>
											</tr>
											<tr>
												<th><strong> <?php esc_html_e('VAT on excise duty:', 'HillebrandGoriEshipping'); ?> </strong></th>
										...
										<?php } else { ?>
											...
											<tr>
												<th><strong> <?php esc_html_e('VAT', 'HillebrandGoriEshipping'); ?> </strong></th>
										...
										<?php if (get_option('VINW_ASSURANCE') == "yes" && $result[0]['insurance'] > 0) { ?>
											<tr>
												<th><strong> <?php esc_html_e('Insurance:', 'HillebrandGoriEshipping') ?> </strong></th>
									...
									<tfoot>
										<th style="border-bottom: none;"><?php esc_html_e('Total:', 'HillebrandGoriEshipping') ?></th>
				...
				<tbody id="packagin">
					<tr class="shipping" data-order_item_id="16">
						<td colspan="3"><strong><?php esc_html_e('Chosen package:', 'HillebrandGoriEshipping') ?> </strong>
							<?php
							switch ($type_package) {
								case 'pallet':
									echo __('pallet', 'HillebrandGoriEshipping');
									break;
								case 'colis':
				...
				<?php $field2 = '<tbody  id="editColisage" style="display:none">
						<tr class="shipping" data-order_item_id="16">
					<td ><strong>' . __('Packing:', 'HillebrandGoriEshipping') . '</strong> </td>

					<td class="name">
				
					<select id="type_colissage" name="type_colissage" class="form-control">
					<option value="choisir" >' . __('select the type', 'HillebrandGoriEshipping') . '</option>		
					<option value="colis" >' . __('Parcels', 'HillebrandGoriEshipping') . '</option>
					'/*<option value="pallet">' . __('Pallet', 'HillebrandGoriEshipping') . </option> option pallet disabled*/
					. '</select>
				...
				if ($offres != null || $type_package == "colis") {
				 ...
				 <tr>
					 ...
					 <tr>
						 <th>' . __('Choice', 'HillebrandGoriEshipping') . '</th>
					
						 <th>' . __('Width', 'HillebrandGoriEshipping') . '</th>
						 <th>' . __('Height', 'HillebrandGoriEshipping') . '</th>
						 <th>' . __('length', 'HillebrandGoriEshipping') . '</th>
						 <th>' . __('weightStill', 'HillebrandGoriEshipping') . '</th>
					...
					<input type="button" class="button " value="' . __('Valider Pallet', 'HillebrandGoriEshipping') . '" id="validPallet"></td></tr>';
				?>
				...
				?> <tbody id="offredispoPallet" style="display:none">
						<tr class="shipping" data-order_item_id="19">
							...
							<td colspan="3"><span style="color: black;font-weight: 600;margin-left: 10px;"><?php esc_html_e('Available offers :', 'HillebrandGoriEshipping') ?></span></td>
						...
						<tr>
							<td colspan="4"></td>
							<td colspan="2">
								<input type="button" class="button " value="<?php esc_html_e('Validate configuration', 'HillebrandGoriEshipping') ?>" id="validEditPallet">
							</td>
						</tr>
					</tbody>
				<?php
				...
				?>
					<tbody id="offredispo" style="display:none">
								...
								if ($type_livraison == "domicile") {
									...
									foreach ($get_rate as $key => $offer) {
										...
										if (preg_match('/Chrono Relais 13H/', $offer['service']) == 0) {
											...
											if ($expedition_type == "export") {
												...
												$offre1 .= '<p>' . __('Estimated tax and duties that will be invoiced by carrier:', 'HillebrandGoriEshipping') . ' <strong>' . sprintf("%01.2f", $tax_amount) . ' ' . $currency_symbol . '</strong></p>';
											}
											if ($result[0]['created_at'] != $result[0]['updated_at']) {
												$offre1 .= '<p>' . __('Price difference:', 'HillebrandGoriEshipping') . ' ' . ($finalPrice - $result[0]['initial_price']) . ' €' . '</p>';
											}
											$offre1 .= '</td></tr>';
										}
										$i++;
									...
									foreach ($get_rate as $key => $offer) {
										...
										if (preg_match('/Chrono Relais 13H/', $offer['service']) == 1) {
											...
											if ($result[0]['created_at'] != $result[0]['updated_at']) {
												$offre2 .= '<p>' . __('Price difference:', 'HillebrandGoriEshipping') . ' ' . ($finalPrice - $result[0]['initial_price']) . ' €' . '</p>';
											}
											$offre2 .= '</td></tr>';
										} else {
											$offre2 = '<tr><td colspan="5">' .  __('No offer available for this parcel point. Please contact Hillebrand Gori eShipping to solve the problem.', 'HillebrandGoriEshipping') . '</td></tr>';
										}
									}
									echo $offre2;
								}
						...
						<tr>
							<td colspan="4"></td>
							<td colspan="2">
								<input type="button" class="button " value="<?php esc_html_e('Validate configuration', 'HillebrandGoriEshipping') ?>" id="validEdit">
							</td>
						</tr>
					</tbody>
				<?php 	}
						...
						<tr class="shipping" data-order_item_id="19">
							...
							<td>
								<?php if ($current_order[0]['offre'] == " ") { ?>
									<p style="color:red; font-weight: bold;"><?php esc_html_e("Carrier offer have been deleted. Please select a new one.", 'HillebrandGoriEshipping') ?> </p>
								<?php } ?>
							</td>
							<td></td>
							<td>
								<?php if ($current_order[0]['offre'] == " ") { ?>
									<input type="button" class="button " value="<?php esc_html_e('Select carrier offer', 'HillebrandGoriEshipping') ?>" id="editoffre">
								<?php } else { ?>
									<input type="button" class="button " value="<?php esc_html_e('Edit expedition', 'HillebrandGoriEshipping') ?>" id="editoffre">
								<?php } ?>
							</td>
							...
							<td>
								...
								<?php if ($type_package == "colis") { ?>
									<input type="button" class="button button-primary" value="<?php esc_html_e('Validate expedition', 'HillebrandGoriEshipping') ?>" <?php echo $disbled ?> id="Expvalidate" style="color:#fff!important">
								<?php } else { ?>
									<input type="button" class="button button-primary" value="<?php esc_html_e('Validate pallet expedition', 'HillebrandGoriEshipping') ?>" <?php echo $disbled ?> id="ExpvalidatePallet" style="color:#fff!important">
...
<?php
	...
	function get_tax_and_duties($carrier, $order_id,  bool $is_zero = false)
	{
		...
		// api request
		$curl = curl_init();
		$url = "https://test.eshipping.hillebrandgori.app/api/shipment/get-duty-and-tax?";
		$url .= "country=" . $country;
		$url .= "&carrier=" . $carrier;
	...
	public function add_parcelpoint_to_admin_order_page()
	{
		...
		if (isset($result[0]['id_exp'])) {
			add_meta_box('HillebrandGoriEshipping-order-details', __('Hillebrand Gori eShipping - Shipment details', 'HillebrandGoriEshipping'), array($this, 'order_details_box'), 'shop_order', 'side', 'default', [$order]);
		}
		...
		if (function_exists('wc_get_order_types')) {
			foreach (wc_get_order_types('order-meta-boxes') as $type) {
				add_meta_box('HillebrandGoriEshipping-order-parcelpoint', __('Hillebrand Gori eShipping - Shipment pickup point', 'HillebrandGoriEshipping'), array($this, 'order_edit_page_parcelpoint'), $type, 'side', 'default');
			}
		} else {
			add_meta_box('HillebrandGoriEshipping-order-parcelpoint', __('Hillebrand Gori eShipping - Shipment pickup point', 'HillebrandGoriEshipping'), array($this, 'order_edit_page_parcelpoint'), 'shop_order', 'side', 'default');
		}
	}
	...
	public function add_tracking_to_admin_order_page()
	{
		...
		if (function_exists('wc_get_order_types')) {
			foreach (wc_get_order_types('order-meta-boxes') as $type) {
				add_meta_box('HillebrandGoriEshipping-order-tracking', __('Hillebrand Gori eShipping - Shipment tracking', 'HillebrandGoriEshipping'), array($this, 'order_edit_page_tracking'), $type, 'normal', 'high');
			}
		} else {
			add_meta_box('HillebrandGoriEshipping-order-tracking', __('Hillebrand Gori eShipping - Shipment tracking', 'HillebrandGoriEshipping'), array($this, 'order_edit_page_tracking'), 'shop_order', 'normal', 'high');
		}
	}
	...
	public function order_edit_page_parcelpoint()
	{
		...
		$parcelpoint_networks = \HillebrandGoriEshipping\HillebrandGoriEshippingConnectWoocommerce\Shipping_Method\Parcel_Point\Controller::get_network_list();
