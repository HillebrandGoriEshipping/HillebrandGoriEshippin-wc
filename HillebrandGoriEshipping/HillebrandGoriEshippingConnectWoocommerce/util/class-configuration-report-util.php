 ...
 *
 * @author    Hillebrand Gori eShipping <api@Vignoblexport.com>
 * @copyright 2007-2020 PrestaShop SA / 2018-2020 Vignoblexport
...
use HillebrandGoriEshipping\HillebrandGoriEshippingConnectWoocommerce\Shipping_Method\Controller;
...
class Configuration_Report_Util
{
	...
	public static function get_configuration_report()
	{
		...
		$report['HillebrandGoriEshipping_config']  = self::get_HillebrandGoriEshipping_config();
	...
	/**
	 * Get all Hillebrand Gori eShipping configurations.
	 *
	 * @return mixed
	 */
	private static function get_HillebrandGoriEshipping_config()
	{
	...
	private static function get_versions()
	{
		...
		$versions['HillebrandGoriEshipping'] = defined('HillebrandGoriEshipping_CONNECT_VERSION') ? HillebrandGoriEshipping_CONNECT_VERSION : null;
	...
	private static function get_shipping_method_details($shipping_method)
	{
		...
		if ('WC_Shipping_Flat_Rate' === $class) {
			...
			$shipping_method_detail['cost']       = $shipping_method->cost;
			$shipping_method_detail['enabled']    = $shipping_method->enabled;
		} elseif ('HillebrandGoriEshipping\\HillebrandGoriEshippingConnectWoocommerce\\Shipping_Method\\Shipping_Method' === $class) {
			$shipping_method_detail['name']       = $shipping_method->title;
			$shipping_method_detail['tax_status'] = $shipping_method->tax_status;
			$shipping_method_detail['enabled']    = $shipping_method->enabled;
