 ...
 *
 * @package     HillebrandGoriEshipping\HillebrandGoriEshippingConnectWoocommerce\Rest_Controller
 */

namespace HillebrandGoriEshipping\HillebrandGoriEshippingConnectWoocommerce\Rest_Controller;

use HillebrandGoriEshipping\HillebrandGoriEshippingConnectWoocommerce\Util\Api_Util;
use HillebrandGoriEshipping\HillebrandGoriEshippingConnectWoocommerce\Util\Auth_Util;
use HillebrandGoriEshipping\HillebrandGoriEshippingConnectWoocommerce\Util\Order_Item_Shipping_Util;
...
class Order
{
	...
	public function order_tracking_event_handler($request, $type)
	{
		...
		if ('shipped' === $type) {
			$shipped_status = get_option('VINW_ORDER_SHIPPED', null);
			$order          = wc_get_order($order_id);
			if (false !== $order) {
				$note = esc_html(__('Your order has been shipped.', 'HillebrandGoriEshipping'));
				$order->add_order_note($note, true);
				$order->save();

				do_action('HillebrandGoriEshipping_connect_order_shipped', $order_id);

				if (null !== $shipped_status && isset($order_statuses[$shipped_status])) {
					$order->update_status($shipped_status);
				} elseif (null !== $shipped_status) {
					update_option('VINW_ORDER_SHIPPED', null);
				}
			}
		}

		if ('delivered' === $type) {
			$delivered_status = get_option('VINW_ORDER_DELIVERED', null);
			$order            = wc_get_order($order_id);
			if (false !== $order) {
				$note = esc_html(__('Your order has been delivered.', 'HillebrandGoriEshipping'));
				$order->add_order_note($note, true);
				$order->save();

				do_action('HillebrandGoriEshipping_connect_order_delivered', $order_id);
				...
				if (null !== $delivered_status && isset($order_statuses[$delivered_status])) {
					...
					update_option('VINW_ORDER_DELIVERED', null);
				}
			}
		}
