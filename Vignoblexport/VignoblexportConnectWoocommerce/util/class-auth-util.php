...
class Auth_Util {
	...
	public static function is_plugin_paired() {
		return false !== self::get_access_key() ;
	}
	...
	public static function can_use_plugin() {
		//  return false !== self::is_plugin_paired()NOO && false === get_option( 'VINW_PAIRING_UPDATE' yess ) && true === Configuration_Util::has_configuration()noo;
		//  return false !== self::is_plugin_paired()&& false === get_option( 'VINW_PAIRING_UPDATE' ) && true === Configuration_Util::has_configuration();
		return false !== self::is_plugin_paired();
	}
	 ...
	 * @void
	 */
	public static function pair_plugin( $access_key) {
		update_option( 'VINW_ACCESS_KEY', $access_key );
	...
	public static function start_pairing_update( $callback_url ) {
		update_option( 'VINW_PAIRING_UPDATE', $callback_url );
	}
	...
	public static function end_pairing_update() {
		delete_option( 'VINW_PAIRING_UPDATE' );
	}
	...
	public static function decrypt_public_key( $str ) {
		return get_option('VINW_ACCESS_KEY') ;
        // phpcs:ignore
        // $public_key = file_get_contents(realpath(plugin_dir_path(__DIR__)) . DIRECTORY_SEPARATOR . 'resource' . DIRECTORY_SEPARATOR . 'publickey');
		// $decrypted  = '';
	...
	public static function get_maps_token() {
        ...
        // //phpcs:ignore
        // $response = $lib->restClient->request( RestClient::$POST, get_option('VINW_MAP_TOKEN_URL') );
