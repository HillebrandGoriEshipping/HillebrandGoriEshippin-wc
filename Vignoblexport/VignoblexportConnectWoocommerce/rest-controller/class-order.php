...
use Vignoblexport\VignoblexportConnectWoocommerce\Util\Order_Item_Shipping_Util;
...
class Order {
	...
	public function run() {
		...
		add_action(
			'rest_api_init', function() {
				register_rest_route(
					'Vignoblexport/v1', '/order/(?P<order_id>[\d]+)/shipped', array(
						'methods'             => 'POST',
		...
		add_action(
			'rest_api_init', function() {
				register_rest_route(
					'Vignoblexport/v1', '/order/(?P<order_id>[\d]+)/delivered', array(
						'methods'             => 'POST',
	...
	public function order_tracking_event_handler( $request, $type ) {
		...
		if ( 'shipped' === $type ) {
			...
			if ( false !== $order ) {
				$note = esc_html( __( 'Your order has been shipped.', 'Vignoblexport' ) );
				$order->add_order_note( $note, true );
				$order->save();

				do_action( 'Vignoblexport_connect_order_shipped', $order_id );
		...
		if ( 'delivered' === $type ) {
			...
			$order            = wc_get_order( $order_id );
			if ( false !== $order ) {
				$note = esc_html( __( 'Your order has been delivered.', 'Vignoblexport' ) );
				$order->add_order_note( $note, true );
				$order->save();

				do_action( 'Vignoblexport_connect_order_delivered', $order_id );
